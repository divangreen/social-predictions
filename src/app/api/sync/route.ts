...
async function syncFromFootballData(leagueId: string, supabase: ReturnType<typeof makeSupabase>) {
  ...
  const fixtures = matches.map(m => {
    ...
    return {
      ...
      status,
      home_score: (status === 'completed' || status === 'live') ? (m.score.fullTime.home ?? null) : null,
      away_score: (status === 'completed' || status === 'live') ? (m.score.fullTime.away ?? null) : null,
...
async function syncFromSportsdb(leagueId: string, season: string, supabase: ReturnType<typeof makeSupabase>) {
  ...
  const fixtures = events.map(e => {
    ...
    return {
      ...
      status,
      home_score: (status === 'completed' || status === 'live') ? homeScore : null,
      away_score: (status === 'completed' || status === 'live') ? awayScore : null,
...
async function syncFromBalldontlie(season: string, supabase: ReturnType<typeof makeSupabase>) {
  ...
  const fixtures = allGames.map(g => {
    ...
    return {
      ...
      kickoff_time: g.date,
      status,
      home_score: (status === 'completed' || status === 'live') ? g.home_team_score : null,
      away_score: (status === 'completed' || status === 'live') ? g.visitor_team_score : null,
